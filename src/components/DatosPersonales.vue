<script setup>
import linkedin from '/src/assets/linkedin_icon.svg';
import github from '/src/assets/github_icon.svg';
import instagram from '/src/assets/instagram_icon.svg';
const title = "Ortiz Eduardo Javier";
const descripcion = "Estudiante de Tecnicatura Universitaria en Programación - UTN";
const residencia = "San Rafael, Mendoza, Argentina";
const presentacion = "Cuento con capacidad para el trabajo en equipo, la comunicación, el aprendizaje continuo y la gestión de los tiempos. Actualmente me encuentro en búsqueda de un nuevo desafío laboral relacionado a mi formación académica donde pueda aplicar mis conocimientos y poder crecer tanto a nivel personal, como profesional. .";
const redesSociales = [
    { id: 1, name: "Linkedin", src: linkedin, url:'https://www.linkedin.com/in/eduardo-javier-ortiz-740264317/'},
    { id: 2, name: "GitHub", src: github, url:'https://github.com/MrEbriO'},
    { id: 3, name: "Instagram",src: instagram, url:'https://www.instagram.com/javi_ortiz092001/'}
];
const telefono = '+54 2604592957';
</script>

<template>
<section class="Datos Personales"> 
    <div class="card">
        <h1>{{title}}</h1>
        <h2>{{descripcion}}</h2>
        <p>{{presentacion}}</p>
        <ul class="container-lista">
            <li v-for="red in redesSociales" :key="red.id">
                <a :href="red.url"><img class="icon-redsocial" :src="red.src" width="35rem" :alt="red.name"></a>
            </li>
        </ul>
        <h3>Mi telefono personal: {{telefono}}</h3>
        <h4> {{residencia}}</h4>

    </div>
</section>
    

</template>

<style scoped>
h1 {
    font-size: 2.5rem;
}
p {
    font-size: 1.2rem;
    font-weight: 600;
    margin-bottom: 1rem;
}
.card {
    background-color: rgb(0, 0, 0);
    border-radius: 10px;
    padding: 10px;
    margin: 10px;
    text-align: center; 
}
.container-lista {
    display: flex;
    justify-content: center;
    list-style: none;
    padding: 0;
    margin: 0;
    text-align: center;
}
.icon-redsocial {
    align-items: center;
    background-color: aliceblue;
    border-radius: 50%;
    padding: 2px;
    margin: 5px;
    box-shadow: 0 0 5px rgba(255, 238, 0, 0.934);
}
.icon-redsocial:hover {
    background-color: rgb(98, 0, 255);
    box-shadow: 0 0 5px rgba(251, 249, 249, 0.934);
}
h3 {
    font-size: 1.2rem;
    font-weight: 600;
}
</style>


